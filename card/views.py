...
def register(request):
    if request.method == 'POST':
        ...
        # get form values
        username = request.POST['username']
        cardscode = request.POST['cardcode']
        ...
        if password == repassword:
            ...
            if User.objects.filter(username = username).exists():
                ...
                user.save()
                card = get_object_or_404(Card, cardcode = cardscode)#Card.objects.filter(cardcode = cardcode)
                card.owner, card.name, card.position, card.company, card.website, card.mail, card.gsm, card.fax, card.instagramacc, card.twitteracc, card.linkedinacc = username, name, position, company, website, email, gsm, fax, instagram, twitter, linkedin
                card.owner = username
                card.save()
...
def editcard(request):
    if request.method == 'POST':
        ...
        cardlist = [card.name, card.position, card.company, card.website, card.mail, card.gsm, card.fax, card.instagramacc, card.twitteracc, card.linkedinacc]
        postlist =[name, position, company, website, email, gsm, fax, instagram, twitter, linkedin]
        ...
        card.name, card.position, card.company, card.website, card.mail, card.gsm, card.fax, card.instagramacc, card.twitteracc, card.linkedinacc = cardlist
        if user is not None:
            card.save()
